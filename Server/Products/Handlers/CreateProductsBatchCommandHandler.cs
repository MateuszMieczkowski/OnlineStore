...
public class CreateProductsBatchCommandHandler : ICommandHandler<CreateProductsBatch>
{
    ...
    private async Task CreateProductAsync(CreateProductDto productDto, IEnumerable<TaxRate> taxRates)
    {
        ...
        var taxRate = taxRates.FirstOrDefault(x => x.Amount == productDto.TaxRateId)
                      ?? throw new NotFoundException($"Tax rate with id {productDto.TaxRateId} was not found."); // TODO back to the previous implementation
